...
- Next you need to copy `docker-compose.yml`, `.env_example`, `setup_service_folders.sh`, `defaults.ini` and `subliminal_defaults.toml` over to this folder.
...
- To set up correct user permissions for the service, run `./setup_service_folders.sh` as the current user. This will create all the necessary folders needed for the service.
...
alias mkv-auto-logs='docker logs -f mkv-auto-service"'
