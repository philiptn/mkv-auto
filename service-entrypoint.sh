...
while true; do
    ...
    if ! pgrep -f 'python3 mkv-auto.py' > /dev/null; then
        ...
        if [ $(ls /mkv-auto/files/input | wc -l) -gt 0 ]; then
            ...
            python3 mkv-auto.py --move --silent --temp_folder /mkv-auto/files/tmp --input_folder /mkv-auto/files/input --output_folder /mkv-auto/files/output $DEBUG_FLAG 2>&1 | tee >(sed 's/\x1b\[[0-9;]*m//g' >> "$log_file")
        fi
    fi
